...
def objective(x,p,pre_H,pre_H1,pre_r1):
    ...
    # initialize H[1]
    H[1]=np.einsum("b,ab->a", f_pre, E_1)**4

    # calculation of H
    for i in range(p-1):
        H[i + 2] = np.einsum(
            "b,c,d,b,d,c,abcd->a", f_pre, f_pre, f_pre, H[i+1], H[i], H[i+1], E_H)**2

    if p==1:
        res=np.einsum("a,b,a,b,c,d,a,b,c,d,abcd->", a[:,p], a[:,p], f_pre, f_pre, f_pre, f_pre, H[p], H[p], H[p-1], H[p-1], E_r1)
    else:
        res=np.einsum("a,b,a,b,c,d,a,b,c,d,abcd->", a[:,p], a[:,p], f_pre, f_pre, f_pre, f_pre, H[p], H[p], H[p-1], H[p-1], E_H)
